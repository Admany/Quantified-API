...
public final class ThreadHealthMonitor {
    ...
    public static void register(Thread thread, ThreadRole role) {
        ...
        Entry entry = new Entry(thread.getId(), thread.getName(), role);
        ENTRIES.put(thread.getId(), entry);
    ...
    public static void unregister(Thread thread) {
        ...
        Entry removed = ENTRIES.remove(thread.getId());
        if (removed != null) {
    ...
    public static void heartbeat(ThreadRole role) {
        ...
        Entry entry = ENTRIES.computeIfAbsent(current.getId(), id -> new Entry(id, current.getName(), role == null ? ThreadRole.UNKNOWN : role));
    ...
    public static void recordCrash(ThreadRole role, Throwable throwable) {
        ...
        Entry entry = ENTRIES.computeIfAbsent(thread.getId(), id -> new Entry(id, thread.getName(), role == null ? ThreadRole.UNKNOWN : role));
