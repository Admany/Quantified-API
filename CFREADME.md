...
<ul style="list-style-type:'→ ';padding-left:20px;color:#e1bee7">
  ...
  <li><b>Caching + persistence layers</b> so repeated expensive work, so your CPU/GPU has more time to nap (for nanoseconds xd)</li>
...
<ul style="list-style-type:'→ ';padding-left:20px;color:#c8e6c9">
  ...
  <li>Backend preferences like <code>preferVulkan()</code>, <code>preferOpenCL()</code>, <code>requireVulkan()</code>, and <code>cpuOnly()</code></li>
</ul>
...
Quantified API gives you a proper runtime instead of forcing you to reinvent multithreading for the 40th time, and then fix each compatability issue, every <b>FREAKING</b> time...
</p>
</div>
...
<ul style="list-style-type:'→ ';padding-left:20px;color:#bbdefb">
  ...
  <li>OpenCL support still exists for compatibility, but Vulkan is the main direction due to it's large advantages over OpenCL</li>
