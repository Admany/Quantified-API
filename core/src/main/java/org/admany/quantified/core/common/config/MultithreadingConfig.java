...
public class MultithreadingConfig {
    ...
    public static boolean isDedicatedServerEnv() {
        // Try to determine server environment via reflection so this class can compile on non-Forge loaders
        try {
            Class<?> fmlEnv = Class.forName("net.minecraftforge.fml.loading.FMLEnvironment");
            java.lang.reflect.Field distField = fmlEnv.getField("dist");
            Object dist = distField.get(null);
            Class<?> distCls = Class.forName("net.minecraftforge.api.distmarker.Dist");
            Object dedicated = resolveEnum(distCls, "DEDICATED_SERVER");
            if (dist != null && dist.equals(dedicated)) {
                return true;
            }
        } catch (Throwable ignored) {
            // not running with Forge present or reflection failed; try ServerLifecycleHooks
        }
        try {
            ...
            if (server != null) {
                Boolean single = (Boolean) server.getClass().getMethod("isSingleplayer").invoke(server);
                return !single;
            }
        } catch (Throwable ignored) {
        }
    ...
    public static Config loadOrCreateConfig(Logger logger) {
        ...
        // Resolve legacy FML paths reflectively (may not be present under other loaders)
        java.nio.file.Path legacyConfigPath = null;
        java.nio.file.Path legacyRootConfigPath = null;
        String legacyPath = "";
        try {
            Class<?> paths = Class.forName("net.minecraftforge.fml.loading.FMLPaths");
            Object cfg = paths.getField("CONFIGDIR").get(null);
            java.nio.file.Path cfgPath = null;
            if (cfg instanceof java.nio.file.Path) cfgPath = (java.nio.file.Path) cfg; else {
                try { cfgPath = (java.nio.file.Path) cfg.getClass().getMethod("get").invoke(cfg); } catch (Throwable ignored) {}
            }
            if (cfgPath != null) {
                legacyPath = cfgPath.resolve("quantified.json").toString();
                legacyConfigPath = cfgPath.resolve("quantified").resolve("quantified_config.json");
            }
            Object game = paths.getField("GAMEDIR").get(null);
            java.nio.file.Path gamePath = null;
            if (game instanceof java.nio.file.Path) gamePath = (java.nio.file.Path) game; else {
                try { gamePath = (java.nio.file.Path) game.getClass().getMethod("get").invoke(game); } catch (Throwable ignored) {}
            }
            if (gamePath != null) {
                legacyRootConfigPath = gamePath.resolve("QuantifiedAPI").resolve("quantified_config.json");
            }
        } catch (Throwable ignored) {
            // FMLPaths not available under this loader
        }
        // Treat as server if we positively detect a dedicated server, even if Dist is misreported
        boolean physIsServer = false;
        try {
            Class<?> fmlEnv = Class.forName("net.minecraftforge.fml.loading.FMLEnvironment");
            java.lang.reflect.Field distField = fmlEnv.getField("dist");
            Object dist = distField.get(null);
            Class<?> distCls = Class.forName("net.minecraftforge.api.distmarker.Dist");
            Object dedicated = resolveEnum(distCls, "DEDICATED_SERVER");
            physIsServer = (dist != null && dist.equals(dedicated));
        } catch (Throwable ignored) {
            physIsServer = false;
        }
    ...
    private static Object resolveEnum(Class<?> type, String name) {
        if (type == null || !type.isEnum() || name == null) {
            return null;
        }
