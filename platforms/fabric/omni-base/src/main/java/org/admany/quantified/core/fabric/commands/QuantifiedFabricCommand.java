...
import org.admany.quantified.core.fabric.wide.FabricCommandDevAccess;
...
public final class QuantifiedFabricCommand {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        try {
            ...
            LOGGER.info("Quantified command tree registered for Fabric omni bootstrap");
        } catch (Throwable t) {
    ...
    private static int webPanel(CommandSourceStack source, String rawState) {
        ...
        if (enable) {
            ...
            LOGGER.info("Fabric omni /quantified webpanel result: success={}, message='{}', url='{}'",
    ...
    private static void deliver(CommandSourceStack source, MutableComponent message, boolean failure) {
        try {
            ...
            if (player != null && deliverToPlayer(player, message)) {
                return;
            }
        } catch (Throwable t) {
            LOGGER.warn("Direct Fabric player delivery lookup failed for Quantified command message", t);
        }
        deliverToSource(source, message, failure);
    }

    private static boolean deliverToPlayer(ServerPlayer player, Component message) {
        if (invokeDisplayClientMessage(player, message)) {
            return true;
        }
        return invokeVoidComponentMethod(player, message, "sendSystemMessage");
    }

    private static void deliverToSource(CommandSourceStack source, Component message, boolean failure) {
        try {
            if (failure) {
                if (invokeVoidComponentMethod(source, message, "sendFailure")) {
                    return;
                }
            } else if (invokeSendSuccess(source, message)) {
                return;
            }
            if (invokeVoidComponentMethod(source, message, "sendSystemMessage")) {
                return;
            }
        } catch (Throwable primaryFailure) {
            LOGGER.warn("Primary Fabric command delivery failed", primaryFailure);
        }
        LOGGER.warn("Unable to deliver Quantified Fabric command message via CommandSourceStack");
    }

    private static boolean invokeDisplayClientMessage(ServerPlayer player, Component message) {
        Method method = findMethod(player.getClass(), "displayClientMessage", Component.class, boolean.class);
    ...
    private static boolean invokeVoidComponentMethod(Object target, Component message, String... names) {
        ...
        for (Method method : target.getClass().getMethods()) {
            if (method.getParameterCount() != 1
                ...
                || !Component.class.isAssignableFrom(method.getParameterTypes()[0])) {
                continue;
            }
            try {
                method.invoke(target, message);
    ...
    private static boolean invokeSendSuccess(CommandSourceStack source, Component message) {
        Method method = findMethod(source.getClass(), "sendSuccess", Supplier.class, boolean.class);
        if (method == null || method.getReturnType() != void.class) {
            return false;
        }
        try {
    ...
    private static int gpuStatus(CommandSourceStack source) {
        ...
        if (vulkanProbeAvailable) {
            ...
            sendInfo(source, "Vulkan runtime: " + (vulkanBindingsPresent ? "Native" : "Isolated bundled") + " | " + (vulkanRuntimeInitialized ? "Ready" : "Deferred until first use"));
        } else {
    ...
    private static boolean requireAdmin(CommandSourceStack source) {
        if (FabricCommandDevAccess.hasDevAccess(source)) {
            return true;
        }
    ...
    private static String describeCommandSource(CommandSourceStack source) {
        ServerPlayer player = source.getPlayer();
        if (player == null) {
