...
public final class QuantifiedForgeWideBootstrap {

    private static final Logger LOGGER = org.slf4j.LoggerFactory.getLogger(QuantifiedForgeWideBootstrap.class);
    private static volatile boolean installed;

    private QuantifiedForgeWideBootstrap() {
    }
    ...
    public static void install(FMLJavaModLoadingContext context) {
        ...
        synchronized (QuantifiedForgeWideBootstrap.class) {
            if (installed) {
                return;
            }
            var modBus = context.getModEventBus();
            modBus.addListener(QuantifiedForgeWideBootstrap::onCommonSetup);
            modBus.addListener(QuantifiedForgeWideBootstrap::onClientSetup);
