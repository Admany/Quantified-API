...
public final class VulkanProbeEntrypoint {
            ...
            for (int i = 0; i < devices.capacity(); i++) {
                ...
                try {
                    ...
                    result.add(new DeviceCandidate(
                        ...
                        computeQueueFamily,
                        info,
                        scoreDevice(deviceType, vendorName, localMemoryBytes, maxComputeInvocations, maxComputeSharedMemoryBytes, softwareAdapter)
                    ));
                } finally {
                    properties.free();
                }
            }
            return sortAndDedupeCandidates(result);
        } finally {
            if (devices != null) {
    ...
    private static BringUpAttempt validateLogicalDeviceBringUp(List<DeviceCandidate> candidates, int apiVersion) {
        ...
        for (DeviceCandidate candidate : autoSelectCandidates(candidates)) {
    ...
    private static List<DeviceCandidate> sortAndDedupeCandidates(List<DeviceCandidate> candidates) {
        ...
        for (DeviceCandidate candidate : candidates) {
            String key = normalizeDeviceKey(candidate.info().name());
            DeviceCandidate existing = unique.get(key);
            if (existing == null || preferCandidate(candidate, existing)) {
                unique.put(key, candidate);
            }
        }
        List<DeviceCandidate> deduped = new ArrayList<>(unique.values());
        deduped.sort(Comparator.comparingDouble(DeviceCandidate::score).reversed());
        return deduped;
    }

    private static List<DeviceCandidate> autoSelectCandidates(List<DeviceCandidate> candidates) {
        List<DeviceCandidate> discrete = candidates.stream()
            .filter(candidate -> !candidate.info().softwareAdapter())
            .filter(candidate -> candidate.info().deviceType() == VK10.VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU)
            .toList();
        return discrete.isEmpty() ? candidates : discrete;
    }

    private static boolean preferCandidate(DeviceCandidate candidate, DeviceCandidate existing) {
        if (candidate.info().softwareAdapter() != existing.info().softwareAdapter()) {
            return !candidate.info().softwareAdapter();
        }
        if (candidate.info().deviceType() != existing.info().deviceType()) {
            return deviceTypeRank(candidate.info().deviceType()) > deviceTypeRank(existing.info().deviceType());
        }
        if (candidate.info().localMemoryBytes() != existing.info().localMemoryBytes()) {
            return candidate.info().localMemoryBytes() > existing.info().localMemoryBytes();
        }
